...
<style>
    ...
    /* Remove qualquer contorno/borda interna ao redor do dia, mês ou ano */
...
st.markdown("---")
            ...
            # Controlo para expandir o tamanho no telemóvel
            col_expandir, _ = st.columns([2, 2])
            with col_expandir:
                modo_expandido = st.toggle("📱 Expandir área de assinatura (Ideal p/ Telemóvel)", value=False)

            # Define as dimensões com base na opção selecionada
            if modo_expandido:
                largura_canvas = 700  # Área ampla e espaçosa
                altura_canvas = 300
            else:
                largura_canvas = 480  # Tamanho padrão compacto
                altura_canvas = 160

            # Moldura visual ao redor do quadro para melhor enquadramento no ecrã
            st.markdown(
                f"""
            ...
            canvas_result = st_canvas(
                ...
                stroke_width=3,  # Traço ligeiramente mais grosso para facilitar o toque no ecrã
                stroke_color="#000000",
                ...
                key=f"canvas_assinatura_{modo_expandido}",  # Chave dinâmica para redesenhar no tamanho certo
            )
            
            st.markdown("</div>", unsafe_allow_html=True)
            st.caption("✍️ Assine com o dedo ou caneta no espaço branco acima.")
